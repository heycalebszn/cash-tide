...
const CurrencySelectDropdown: React.FC<CurrencySelectDropdownProps> = ({ isOpen, onSelect, selectedCurrencyCode, onClose }) => {
  ...
  return (
    <div className="absolute top-full left-0 z-10 w-full max-w-sm bg-white rounded-xl shadow-lg mt-2 py-2">
      {currencies.map((currency) => (
        <div
          key={currency.code}
          className={`flex items-center justify-between p-3 cursor-pointer hover:bg-gray-100 ${selectedCurrencyCode === currency.code ? 'bg-red-500 text-white' : ''}`}
          onClick={() => {
